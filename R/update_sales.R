...
update_sales <- function(competition, simperiod, base_market){
  ...
  for (company in names(competition)){
    ...
    unit_cost <- company_data$census$finished_products %>%
      ...
      dplyr::mutate(unit_cost = round(value / quantity,2)) %>%
    ...
    sales <- company_data$profile %>%
      ...
      dplyr::mutate(date = end_date, vat = base_market$environments$value_added_tax) %>%
    ...
    inventory_out <- sales %>%
      ...
      dplyr::mutate(quantity = -quantity, value = -value) %>%
